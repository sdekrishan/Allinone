let container = document.querySelector("#container");
    ...
    filter.addEventListener("change", () => {
      let filterUrl = `https://mockfour-brown.vercel.app/books?genre=${filter.value}`;
      get_books(filterUrl);
    });

    sort.addEventListener("change", () => {
      console.log(sort.value);
      let filterUrl = `https://mockfour-brown.vercel.app/books?_sort=cost&_order=${sort.value}`;
      get_books(filterUrl);
    });
    ...
    function display(arr) {
      ...
      arr.forEach((book, ind) => {
        ...
        // book_borrow_button.style.backgroundColor='#2E4F4F'
        book_borrow_button.setAttribute("class","butn")
        book_div.append(
          book_img,
          book_name,
          book_author,
